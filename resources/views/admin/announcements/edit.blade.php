                    ...
                    <form action="{{ route('admin.announcements.update', $announcement) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="content" id="content" rows="6" required
                                      class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('content', $announcement->content) }}</textarea>
                            @error('content')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            @if(!$announcement->published_at)
                                <div class="flex items-center">
                                    <input type="checkbox" name="publish_now" id="publish_now" value="1"
                                           class="rounded border-gray-300 dark:border-gray-600 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                                    <label for="publish_now" class="ml-2 text-sm text-gray-700 dark:text-gray-300">Publish now</label>
                                </div>
                            @else
                                <p class="text-sm text-gray-600 dark:text-gray-400">Published on {{ $announcement->published_at->format('M d, Y h:i A') }}</p>
                            @endif
                        </div>
